...
тут стоит пояснить, что на некоторых интерфейсах dhcp выключенн, поэтому надо смотреть на интерфейс где написано dynamic и пинговать его ip. На скрине показано, что машина w22 может пинговать ip, который выдал DHCP сервер.
1
